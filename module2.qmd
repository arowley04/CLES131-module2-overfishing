...
# Join bioparams with its metrics
bio_with_metrics <- bioparams %>%
  left_join(bioparamsmetrics, 
            by = "bioid")

colnames(bioparams)
colnames(bioparamsmetrics)
...
```{r}
cod_metrics <- c("Total landings", "Spawning Stock Biomass")
...
cod_ts_plot <- ts_with_metrics %>%
  ...
  left_join(stock %>% select(stockid, region), by = "stockid")
cod_ts_plot
```
